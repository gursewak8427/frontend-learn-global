...
// const ENDPOINT = "https://learnglobal.co.in:3006/"; // ok For Live
// const ENDPOINT = "https://44.215.39.78:3006/";
...
const AddSchools = () => {
  ...
  const openFile = async (key, value) => {
    ...
    var response2 = await axios.get(
      process.env.REACT_APP_NODE_URL + "/admin/schoolnames"
    );
    ...
    Papa.parse(value, {
      ...
      complete: function (results) {
        setState({
          ...
          schoolsList: response2.data.details.list.map(
        ...
        <>
          <div className="">
            {state.fileUploadedStatus ? (
              ...
              <div className="flex flex-column items-center w-10/12 mx-auto mt-10 mb-3">
            ...
            {state.fileUploadedStatus ? (
              <div className="tableData">
                <label className="flex items-center justify-between" htmlFor="">
                  <b>
                    <span className="m-1">School Details</span>
                  </b>
                      ...
                      onClick={(_) => {
                        if (window.confirm("Are you want to close")) {
                          ...
                          document
                            ...
                            .classList.remove("hide_now");
                          document
                            .getElementsByTagName("main")[0]
                            .classList.remove("full_screen");
                        }
                      }}
                    >
                      Close
                ...
                <table className="table schoolList">
                  ...
                  <tbody>
                    {state.fileUploadedStatus ? (
                      state.csvData[1].map((schoolDataArr, index) => {
                        // console.log(schoolDataArr[0])
                        let thisrow = false;
                        if (schoolDataArr.length == 1 && schoolDataArr[0] == "")
                          return;

                        let isDoubleProgram = -1;
                        if (
                          visitedSchoolPrograms.includes(
                            `${schoolDataArr[0]}-${schoolDataArr[14]}`
                          )
                        ) {
                          isDoubleProgram = visitedSchoolPrograms.indexOf(
                            `${schoolDataArr[0]}-${schoolDataArr[14]}`
                          );
                          if (!thisrow) {
                            errorFieds++;
                          ...
                          >
                            ...
                            {schoolDataArr.map((item, index) => {
                              ...
                              if (
                                ...
                                return (
                                  <td
                                    title={`Unknown School Name : ${item}`}
                                    className="p-2 myCell required_missing"
                                  >
                                    {item}
                                  </td>
                                );
