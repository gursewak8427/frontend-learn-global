...
const StudentDocumentsPopup = ({ sId, fId, setModalCode }) => {
    ...
    useEffect(() => {
        if (sId) {
            ...
            axios.get(process.env.REACT_APP_NODE_URL + "/student?id=" + sId, config).then((res) => {
                ...
                axios.post(process.env.REACT_APP_NODE_URL + "/student/getdocuments", { fileId: fId, student_id: sId }, config).then((documentsResponse) => {
    ...
    const approveFileNow = () => {
        ...
        if (rejectedDocs.length != 0) {
            navigate("/d/admin/documents-rejected-files")
            setModalCode(false)
            return;
        }
    ...
    const approveProfile = () => {
        ...
        axios.post(process.env.REACT_APP_NODE_URL + "/student/approveProfile?id=" + sId, data, config).then((res) => {
            if (res.data.status == "1") {
                ...
                setState({
                    ...state,
                    studentProfileprofileApproveLoading: false
                });
                return;
            }
            alert(res.data.message);
