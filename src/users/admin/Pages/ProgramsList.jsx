...
>>>>>>> 1364d015a9aa03628ea1ad4108e88de5fd5c486d
    ...
    const toggleTopStatus = (sId, pId) => {
        const config = { headers: { "Authorization": `Bearer ${state.adminToken}` } }
        let data = { sId, pId }
        axios.post(process.env.REACT_APP_NODE_URL + "/admin/toggletopstatus", data, config).then(res => {
            console.log({ res })
        }).catch(err => {
            console.log(err.response.data)
            // alert(err.response.data.message)
...
<<<<<<< HEAD
                                                                    <td className="border-2 p-2 text-center">
                                                                        {
                                                                            Boolean(program.top_status) != false ?
                                                                                <Switch color="primary" defaultChecked onClick={(e) => {
                                                                                    if (window.confirm("Are you sure ?")) {
                                                                                        toggleTopStatus(school._id, program.program_name)
                                                                                    } else {
                                                                                        e.preventDefault();
                                                                                        e.stopPropagation();
                                                                                        return false;
                                                                    ...
                                                                    <td className="border-2 p-2 text-center">
                                                                        <button
                                                                            onClick={() => navigate(`/d/admin/intakes_program?schoolId=${school._id}&programId=${program.program_id}`)}
