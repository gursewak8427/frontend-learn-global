...
const AgentStudentDocuments = (props) => {
    ...
    useEffect(() => { // get docs list
        ...
        axios.post(process.env.REACT_APP_NODE_URL + "/student/getdocuments", { "student_id": studentId }, config).then((res) => {
            console.log(res);
            setState({
                ...state,
    ...
    const submitAllDocs = () => {
        ...
        axios.post(process.env.REACT_APP_NODE_URL + "/student/submitAllDocs", { "student_id": studentId }, config).then((res) => {
            console.log(res);
            if (res.data.details.status == "1") {
                toast("Documents has been submitted")
                return;
            }
